...
- Never trust client-controlled cookies for authentication without cryptographic signing (e.g. HMAC).
- The client-side JS and server-side validation used **different decoding logic**, which was the key to finding the correct payload format.
- Always enumerate all linked pages — the flag page was linked directly from the main page.
